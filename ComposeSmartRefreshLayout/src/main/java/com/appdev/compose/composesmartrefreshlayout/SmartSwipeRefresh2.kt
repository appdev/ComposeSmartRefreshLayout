    ...
    Box(modifier = modifier.zIndex(-1f)) {
        SubComposeSmartSwipeRefresh(
            ...
            Box(
                ...
                if (isNeedRefresh) {
                    Box(Modifier.offset(y = getOffSetY(header, state))) {
                        headerIndicator()
                    }
                }
                // 因为无法测量出content的高度 所以footer偏移到content布局之下
                Log.d("indicatorOffset", "SmartSwipeRefresh2 state.indicatorOffset ${state.indicatorOffset}")
...
private const val TAG = "SmartSwipeRefresh2"
@Composable
